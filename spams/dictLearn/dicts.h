...
/// Constructor with data
template <typename T> Trainer<T>::Trainer(const long k, const
      long batchsize, const long NUM_THREADS) : _k(k), 
...
template <typename T> Trainer<T>::Trainer(const Matrix<T>& D, 
      const long batchsize, const long NUM_THREADS) : _k(D.n()),
...
template <typename T> Trainer<T>::Trainer(const Matrix<T>& A, const Matrix<T>&
      B, const Matrix<T>& D, const long itercount, const long batchsize, 
      const long NUM_THREADS) : _k(D.n()),_initialDict(true),_itercount(itercount),
...
void Trainer<T>::train(const Data<T>& X, const ParamDictLearn<T>& param) {
   ...
   T rho = param.rho;
   T t0 = param.t0;
   long sparseD = param.modeD == L1L2 ? 2 : param.modeD == L1L2MU ? 7 : 6;
   long NUM_THREADS=init_omp(_NUM_THREADS);
   if (param.verbose) {
   ...
   const long K = _k;
   const long n = X.m();
   const long L = param.mode == SPARSITY ? static_cast<long>(param.lambda) : 
      param.mode == PENALTY && param.lambda == 0 && param.lambda2 > 0 && !param.posAlpha ? K : MIN(n,K);
   const long batchsize= param.batch ? M : MIN(_batchsize,M);
   ...
   long JJ = param.iter < 0 ? 100000000 : param.iter;
   bool even=true;
   long last_written=-40;
   long i;
   for (i = 0; i<JJ; ++i) {
      if (param.verbose) {
      ...
      if (param.log) {
         long seconds=static_cast<long>(floor(log(time.getElapsed())*5));
         if (seconds > last_written) {
            last_written++;
            sprintf(buffer_string,"%s_%ld.log",param.logName,
                  last_written+40);
...
#pragma omp parallel for private(j)
      for (j = 0; j<batchsize; ++j) {
         ...
         long numT=0;
#endif
         const long index=perm[(j+i*batchsize) % M];
         Vector<T>& Xj = XT[numT];
      ...
      if (param.batch) {
         ...
         long ii=_itercount+i;
         long num_elem=MIN(2*M, ii < batchsize ? ii*batchsize :
               batchsize*batchsize+ii-batchsize);
         T scal2=T(T(1.0)/batchsize);
         T scal;
         long totaliter=_itercount+count;
...
template <typename T>
void writeLog(const Matrix<T>& D, const T time, long iter, 
      char* name) {
   std::ofstream f;
   f.precision(12);
...
void Trainer<T>::trainOffline(const Data<T>& X, 
   ...
   long sparseD = param.modeD == L1L2 ? 2 : 6;
   long J = param.iter;
   long batch_size= _batchsize;
   long batchsize= _batchsize;
   long NUM_THREADS=init_omp(_NUM_THREADS);
   ...
   fprintf(stderr,"num param iterD: %ld\n",param.iter_updateD);
...
#pragma omp parallel for private(j)
      for (j = 0; j<batch_size; ++j) {
         ...
         long numT=0;
#endif
         const long ind=perm[(j+i*batch_size) % M];
         Vector<T> Xj, coeffj;
         SpVector<T>& spcoeffj = spcoeffT[numT];
         Vector<T>& DtRj = DtRT[numT];
