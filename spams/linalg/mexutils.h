...
template <> inline bool mexCheckType<long>(const mxArray* array) {
...
bool checkSize(const mxArray* array, const long m, const long n) {
   const mwSize* dims=mxGetDimensions(array);
   long _m=static_cast<long>(dims[0]);
   long _n=static_cast<long>(dims[1]);
   return _n==n && _m==m;
};
...
template <typename T>
void createCopySparse(T*& alpha_v2, long*& alpha_r2, long*& alpha_pB2, long*& alpha_pE2,
      double* alpha_v, mwSize* alpha_r, mwSize* alpha_pB, mwSize* alpha_pE, long M) {
   ...
   if (typeid(alpha_r2) == typeid(alpha_r)) {
      alpha_r2=reinterpret_cast<long*>(alpha_r);
      alpha_pB2=reinterpret_cast<long*>(alpha_pB);
      alpha_pE2=reinterpret_cast<long*>(alpha_pE);
   } else {
      alpha_r2= new long[alpha_pB[M]];
      for (mwSize i = 0; i<alpha_pB[M]; ++i) alpha_r2[i]=static_cast<long>(alpha_r[i]);
      alpha_pB2= new long[M+1];
      for (long i = 0; i<=M; ++i) alpha_pB2[i]=static_cast<long>(alpha_pB[i]);
...
template <typename T>
inline void deleteCopySparse(T*& alpha_v2, long*& alpha_r2, long*& alpha_pB2, long*& alpha_pE2,
...
template <typename T>
inline mxArray* createMatrix(long m, long n);

/// Create a m x n double matrix
template <> inline mxArray* createMatrix<double>(long m, long n) {
   return mxCreateNumericMatrix(static_cast<mwSize>(m),
         static_cast<mwSize>(n),mxDOUBLE_CLASS,mxREAL);
};

/// Create a m x n float matrix
template <> inline mxArray* createMatrix<float>(long m, long n) {
   return mxCreateNumericMatrix(static_cast<mwSize>(m),
...
template <typename T>
inline mxArray* createImage(long h, long w, long V);

/// Create a h x w x V double image
template <>
inline mxArray* createImage<double>(long h, long w, long V) {
   if (V ==1) {
...
template <>
inline mxArray* createImage<float>(long h, long w, long V) {
   if (V ==1) {
...
template <typename T> inline void convertSpMatrix(mxArray*& matlab_mat, long K,
      long M, long n, long nzmax, const T* v, const long* r, const long* pB) {
   matlab_mat=mxCreateSparse(K,M,nzmax,mxREAL);
   double* Pr=mxGetPr(matlab_mat);
   for (long i = 0; i<nzmax; ++i) Pr[i]=static_cast<double>(v[i]);
   mwSize* Ir=mxGetIr(matlab_mat);
   for (long i = 0; i<nzmax; ++i) Ir[i]=static_cast<mwSize>(r[i]);
   mwSize* Jc=mxGetJc(matlab_mat);
   if (n == 0) return;
   for (long i = 0; i<=n; ++i) Jc[i]=static_cast<mwSize>(pB[i]);
};
