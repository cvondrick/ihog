...
extern "C" {
#endif
   size_t cblas_idamin( long n,  double* X,  long incX);
   size_t cblas_isamin( long n,  float* X,  long incX);
#ifdef HAVE_MKL
};
#endif
...
#ifdef HAVE_MKL
extern "C" {
   void vdSqr( long n,  double* vecIn, double* vecOut);
   void vsSqr( long n,  float* vecIn, float* vecOut);
   void vdSqrt( long n,  double* vecIn, double* vecOut);
   void vsSqrt( long n,  float* vecIn, float* vecOut);
   void vdInvSqrt( long n,  double* vecIn, double* vecOut);
   void vsInvSqrt( long n,  float* vecIn, float* vecOut);
   void vdSub( long n,  double* vecIn,  double* vecIn2, double* vecOut);
   void vsSub( long n,  float* vecIn,  float* vecIn2, float* vecOut);
   void vdDiv( long n,  double* vecIn,  double* vecIn2, double* vecOut);
   void vsDiv( long n,  float* vecIn,  float* vecIn2, float* vecOut);
   void vdExp( long n,  double* vecIn, double* vecOut);
   void vsExp( long n,  float* vecIn, float* vecOut);
   void vdInv( long n,  double* vecIn, double* vecOut);
   void vsInv( long n,  float* vecIn, float* vecOut);
   void vdAdd( long n,  double* vecIn,  double* vecIn2, double* vecOut);
   void vsAdd( long n,  float* vecIn,  float* vecIn2, float* vecOut);
   void vdMul( long n,  double* vecIn,  double* vecIn2, double* vecOut);
   void vsMul( long n,  float* vecIn,  float* vecIn2, float* vecOut);
   void vdAbs( long n,  double* vecIn, double* vecOut);
   void vsAbs( long n,  float* vecIn, float* vecOut);
}
#endif
...
template <typename T> long cblas_iamin( INTT n,  T* X,  INTT incX);
/// interface to cblas_i*amax
template <typename T> long cblas_iamax( INTT n,  T* X,  INTT incX);
...
/// interface to v*Sqr
template <typename T> void vSqrt( long n,  T* vecIn, T* vecOut);
/// interface to v*Sqr
template <typename T> void vInvSqrt( long n,  T* vecIn, T* vecOut);
/// interface to v*Sqr
template <typename T> void vSqr( long n,  T* vecIn, T* vecOut);
/// interface to v*Sub
template <typename T> void vSub( long n,  T* vecIn,  T* vecIn2, T* vecOut);
/// interface to v*Div
template <typename T> void vDiv( long n,  T* vecIn,  T* vecIn2, T* vecOut);
/// interface to v*Exp
template <typename T> void vExp( long n,  T* vecIn, T* vecOut);
/// interface to v*Inv
template <typename T> void vInv( long n,  T* vecIn, T* vecOut);
/// interface to v*Add
template <typename T> void vAdd( long n,  T* vecIn,  T* vecIn2, T* vecOut);
/// interface to v*Mul
template <typename T> void vMul( long n,  T* vecIn,  T* vecIn2, T* vecOut);
/// interface to v*Abs
template <typename T> void vAbs( long n,  T* vecIn, T* vecOut);
...
//template <typename T> void lasrt2(char& id,  INTT& n, T *d, long* key);
...
/// Implementation of the INTTerface for cblas_scopy
template <> inline void cblas_copy<long>( INTT n,  long* X,  INTT incX, 
      long* Y,  INTT incY) {
   for (long i = 0; i<n; ++i)
      Y[incY*i]=X[incX*i];
};
...
/// Implementation of the INTTerface for cblas_saxpy
template <> inline void cblas_axpy<long>( INTT n,  long a,  long* X,
       INTT incX, long* Y,  INTT incY) {
   for (long i = 0; i<n; ++i)
      Y[i] += a*X[i];
};
...
/// Implementation of the INTTerface for cblas_sscal
template <> inline void cblas_scal<long>( INTT n,  long a, long* X, 
       INTT incX) {
...
template <> inline long cblas_dot<long>( INTT n,  long* X,
       INTT incX,  long* Y, INTT incY) {
   long total=0;
   long i,j;
   j=0;
...
///  Implementation of the INTTerface for cblas_sgemv
template <> inline void cblas_gemv<long>( CBLAS_ORDER order,
       CBLAS_TRANSPOSE TransA,  INTT M,  INTT N,
       long alpha,  long *A,  INTT lda,
       long *X,  INTT incX,  long beta,
      long *Y,  INTT incY) {
   ///  not implemented
};
///  Implementation of the INTTerface for cblas_sgemv
...
template <> inline void cblas_gemm<long>( CBLAS_ORDER order, 
       CBLAS_TRANSPOSE TransA,  CBLAS_TRANSPOSE TransB, 
       INTT M,  INTT N,  INTT K,  long alpha, 
       long *A,  INTT lda,  long *B,  INTT ldb,
       long beta, long *C,  INTT ldc) {
   /// not implemented
};
///  Implementation of the INTTerface for cblas_sgemm
...
///  Implementation of the INTTerface for cblas_ssyrk
template <> inline void cblas_syrk<long>( CBLAS_ORDER order, 
       CBLAS_UPLO Uplo,  CBLAS_TRANSPOSE Trans,  INTT N,  INTT K,
       long alpha,  long *A,  INTT lda,
       long beta, long *C,  INTT ldc) {
   /// not implemented
...
template <> inline long cblas_iamax<double>( INTT n,  double* X,
       INTT incX) {
   //return cblas_idamax(n,X,incX);
   return static_cast<long>(idamax_(&n,X,&incX)-1);
};
///  Implementation of the interface for cblas_isamax
template <> inline long cblas_iamax<float>( INTT n,  float* X, 
       INTT incX) {
   //return cblas_isamax(n,X,incX);
   return static_cast<long>(isamax_(&n,X,&incX)-1);
};
...
//, INTT* ipiv, double* work) {
   ...
   delete[](query);
   work = new double[static_cast<long>(lwork)];
   dsytrf_(&uplo,&n,a,&lda,ipiv,work,&lwork,&info);
   delete[](work);
   work = new double[static_cast<long>(2*n)];
   dsytri_(&uplo,&n,a,&lda,ipiv,work,&info);
   delete[](work);
   delete[](ipiv);
};
...
template <> inline void sytri<float>(char& uplo, INTT n, float* a, INTT lda) {
   ...
   lwork=static_cast<INTT>(*query); 
   delete[](query);
   work = new float[static_cast<long>(lwork)];
   ssytrf_(&uplo,&n,a,&lda,ipiv,work,&lwork,&info);
   delete[](work);
   work = new float[static_cast<long>(2*n)];
...
//template <> inline void lasrt2(char& id, INTT& n, double *d,long* key) {
//   //dlasrt2_(&id,const_cast<long*>(&n),d,key,&info);
//   dlasrt2(&id,&n,d,key,&info);
//};
///// interaface to *lasrt
//template <> inline void lasrt2(char& id, INTT& n, float *d, long* key) {
...
template <> void inline gesvd( char& jobu, char& jobvt, INTT m, 
   ...
   lwork=static_cast<INTT>(*query); 
   delete[](query);
   double* work = new double[static_cast<long>(lwork)];
   dgesvd_(&jobu, &jobvt, &m, &n, a, &lda, s, u, &ldu, vt, &ldvt,
         work, &lwork, &info );
   delete[](work);
}
template <> void inline gesvd( char& jobu, char& jobvt, INTT m, 
   ...
   lwork=static_cast<INTT>(*query); 
   delete[](query);
   float* work = new float[static_cast<long>(lwork)];
   sgesvd_(&jobu, &jobvt, &m, &n, a, &lda, s, u, &ldu, vt, &ldvt,
         work, &lwork, &info );
   delete[](work);
}
...
/// cblas_idamin
template <> inline long cblas_iamin<double>( long n,  double* x,
       long incx) {
   return (int) cblas_idamin(n,x,incx);
};
/// implemenation of the interface of the non-offical blas, level 1 function 
/// cblas_isamin
template <> inline long cblas_iamin<float>( long n,  float* x, 
       long incx) {
...
template <typename T> inline void vSqr( long n,  T* vecIn, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=vecIn[i]*vecIn[i];
};
template <typename T> inline void vSqrt( long n,  T* vecIn, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=sqr<T>(vecIn[i]);
};
template <typename T> inline void vInvSqrt( long n,  T* vecIn, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=T(1.0)/sqr<T>(vecIn[i]);
};

/// Slow implementation of vdSub and vsSub
template <typename T> inline void vSub( long n,  T* vecIn1, 
       T* vecIn2, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=vecIn1[i]-vecIn2[i];
};
/// Slow implementation of vdInv and vsInv
template <typename T> inline void vInv( long n,  T* vecIn, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=1.0/vecIn[i];
};
/// Slow implementation of vdExp and vsExp
template <typename T> inline void vExp( long n,  T* vecIn, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=exp(vecIn[i]);
};
/// Slow implementation of vdAdd and vsAdd
template <typename T> inline void vAdd( long n,  T* vecIn1, 
       T* vecIn2, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=vecIn1[i]+vecIn2[i];
};
/// Slow implementation of vdMul and vsMul
template <typename T> inline void vMul( long n,  T* vecIn1, 
       T* vecIn2, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=vecIn1[i]*vecIn2[i];
};
/// Slow implementation of vdDiv and vsDiv
template <typename T> inline void vDiv( long n,  T* vecIn1, 
       T* vecIn2, T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=vecIn1[i]/vecIn2[i];
};
/// Slow implementation of vAbs
template <typename T> inline void vAbs( long n,  T* vecIn, 
      T* vecOut) {
   for (long i = 0; i<n; ++i) vecOut[i]=abs<T>(vecIn[i]);
};

/// Slow implementation of cblas_idamin and cblas_isamin
template <typename T> long inline cblas_iamin(INTT n, T* X, INTT incX) {
   long imin=0;
   double min=fabs(X[0]);
