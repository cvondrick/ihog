...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin, int normalize, double truncator) {
  ...
  for (int x = 0; x < out[1]; x++) {
    for (int y = 0; y < out[0]; y++) {
      ...
      for (int o = 0; o < 18; o++) {
	double h1 = min(*src * n1, truncator);
	double h2 = min(*src * n2, truncator);
	double h3 = min(*src * n3, truncator);
	double h4 = min(*src * n4, truncator);
	*dst = 0.5 * (h1 + h2 + h3 + h4);
	t1 += h1;
	t2 += h2;
	t3 += h3;
	...
	src += blocks[0]*blocks[1];
      ...
      for (int o = 0; o < 9; o++) {
        ...
        double h1 = min(sum * n1, truncator);
        double h2 = min(sum * n2, truncator);
        double h3 = min(sum * n3, truncator);
        double h4 = min(sum * n4, truncator);
        *dst = 0.5 * (h1 + h2 + h3 + h4);
        dst += out[0]*out[1];
        src += blocks[0]*blocks[1];
      }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  if (nrhs >= 3) {
    normalize = (int)mxGetScalar(prhs[2]);
  }
  if (nrhs >= 4) {
    truncator = (double)mxGetScalar(prhs[3]);
  }

  plhs[0] = process(prhs[0], prhs[1], normalize, truncator);
}
